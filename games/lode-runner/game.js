(function() {
    ...
    function init() {
        ...
        try { const s = localStorage.getItem('pixelarcade.lode-runner.highscore'); if (s) highScore = parseInt(s); } catch(e){}
    ...
    function gameOver() { 
        gameState = 'gameover'; 
        if(score>highScore){highScore=score;try{localStorage.setItem('pixelarcade.lode-runner.highscore',highScore);}catch(e){}} 
    }
    ...
    function render() {
        ...
        if (gameState === 'start') {
            ...
            ctx.fillText('LODE RUNNER', CONFIG.CANVAS_WIDTH/2, CONFIG.CANVAS_HEIGHT/2 - 30);
